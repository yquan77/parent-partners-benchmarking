...
- [ ] 建立马来文翻译治理机制，避免已审定用词反复漂移：
  - 每张slide改用永久 `data-slide-id`，翻译以ID对应，不再只靠数组页码
  - 翻译加入 `draft / reviewed / locked / stale` 状态；已锁定条目不得被全套润色顺手改写
  - 保存中文原文指纹；中文有变才自动标记对应国文为 `stale`
  - 审核页改成中马文并排，并在commit前检查缺译、多译、过期翻译与手机溢出
