...
- [x] 学术引用逐条核对出处（Sarana Ibu Bapa MOE 2012、mutualistic goals、boundary-spanning、GRR、Adams & Christenson信任研究）
...
- [x] 人形icon组件（`.people-icons`）：5位起点、7组×5人分身克隆动画（原地变灰+克隆体飞入）、278人次密集阵列
- [x] 荧光盾牌揭晓：讲者点击触发（非自动倒数），实测两次点击行为正常
- [x] 现场互动投票：Supabase即时星级评分（1-5分「你现在几分想回去试试看」），QR码用vendored qrcode.min.js生成，见下方Supabase章节
- [x] 全部46张slide（内容持续在长，以index.html实际slide数为准）
- [ ] **马来文同步显示功能**（新增待办，见 handoff.md「下一步」，需要先做内容翻译，技术方案已想清楚待执行）
...
├── index.html              # 主deck，46张slide，看顶部HTML注释了解排版规则
├── assets/
...
- **今天新建了一个全局skill**：`~/.claude/skills/html-slide-deck/`，把这次做deck过程中反复调整出来的设计原则（一页一重点、全屏照片不用小方框、文字挡到照片主体时怎么切换排版、招牌动画讲者手动触发、克隆动画正确做法等）连同现成CSS/JS片段都写进去了。**以后任何专案要做类似的叙事型HTML slide deck，会自动触发这个skill，不用重新摸索一次，直接去读 `references/css-patterns.md` 拿现成代码改。**
- 跟 Yong Quan 博士研究（IBSDK）的关系见 handoff.md 「与博士研究的关系」章节
